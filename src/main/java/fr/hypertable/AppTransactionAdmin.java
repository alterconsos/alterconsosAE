	...
	AppTransactionAdmin(String body, byte[] file) throws AppException {
		...
		if ("dumpV".equals(op)) {
			try {
				...
				byte[] bytes = sbx.toString().getBytes(StandardCharsets.UTF_8);
				zos.write(bytes);
				zos.close();
...
//			}
			...
			String sb = "{\"status\":\"OK\", \"bytes\":" + totLg +
					", \"cells\":" + totCe +
					", \"nodes\":" + totNb +
					", \"mimes\":" + totMi + "}";
				...
				if ("PHOTO".equals(type)) {
					...
					try {
						...
						if (cc != null) { // si null, cell purgée mais encore présente dans Versions
							...
							bytes = sbx.toString().getBytes(StandardCharsets.UTF_8);
						} else {
							absents.add(Versions.keyOf(type, col));
							continue;
						}		
	...
	private byte[] dumpLineS(String line, String ver, String cols, String types)
		...
		try {
			...
			bos2.write(dumper.minVersion.getBytes(StandardCharsets.UTF_8));
			bos2.write(0);
			bos.writeTo(bos2);
