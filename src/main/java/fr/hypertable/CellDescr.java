...
public class CellDescr {
	...
	private final ArrayList<NodeDescr> nodeDescrs = new ArrayList<NodeDescr>();
	...
	public CellDescr(Class<?> clazz) {
		...
		Method met = null;
		try {
			met = clazz.getDeclaredMethod("cellDescr");
	...
	private class NodeDescr {
		...
		private String name;
		private byte id;
		private final FieldDescr[] fieldDescrs = new FieldDescr[100];
		...
		private String toJSON(Object target, boolean admin, int[] fields)
			...
			for (int j = 0; j < fieldsIds.length; j++) {
				...
				case 6: { // byte[]
					...
					sb.append("\"");
					AcJSON.escape(Base64.getEncoder().encodeToString(b), sb);
					sb.append("\"");
					break;
				}
				case 7: { // ArrayShort
			...
			private void writeObj(DataOutputStream out, Object value) {
				try {
					...
					case 6: { // byte[]
						...
						byte[] b = Base64.getDecoder().decode(x);
