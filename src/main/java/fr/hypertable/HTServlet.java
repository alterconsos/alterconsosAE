...
public class HTServlet extends HttpServlet {
	...
	@Override public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			byte[] b = getResource(ctx, "/WEB-INF/keys.properties");
			keys = new Properties();
			if (b != null) keys.load(new ByteArrayInputStream(b));
			...
			String s = new String(appHtml, StandardCharsets.UTF_8);
			int i = s.indexOf("${maps.gooleapis}");
			String deb = s.substring(0, i);
			String fin = s.substring(i - 1 + "${mapsgoogle.apis}".length());
			StringBuffer sb = new StringBuffer();
			sb.append(deb);
			sb.append(keys.getProperty("mapsgoogle.apis"));
			sb.append(fin);
			String s2 = sb.toString();
			appHtml = s2.getBytes(StandardCharsets.UTF_8);

			byte[] acjson = getResource(ctx, "/WEB-INF/ac.json");
			arg = AcJSON.parseObjectEx(new String(acjson, StandardCharsets.UTF_8));
	...
	public void page(HttpServletRequest req, HttpServletResponse resp)
			throws IOException {
		ByteArrayOutputStream bos = new ByteArrayOutputStream();
		bos.write(html1.getBytes(StandardCharsets.UTF_8));
		bos.write(contextPath.getBytes(StandardCharsets.UTF_8));
		bos.write(html1b.getBytes(StandardCharsets.UTF_8));
		String x = appScript(req, req.getQueryString());
		bos.write(x.getBytes(StandardCharsets.UTF_8));
		bos.write(appHtml);
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if (!onoff && !uri.startsWith("/admin")) {
			...
			resp.getOutputStream().write(r.getBytes(StandardCharsets.UTF_8));
			return;
		}
	...
	private void errDoc(HttpServletResponse resp, String m) throws IOException{
		resp.setContentType("text/plain");
		resp.setCharacterEncoding("UTF-8");
...
//		if (!onoff && !uri.startsWith("/dumpload") && !uri.startsWith("/admin")) {
		...
		if (uri.startsWith("/import/")) {
			...
			args = new String(getArg1(req), StandardCharsets.UTF_8);
		}
