...
public class AppConfig implements IAppConfig {
	...
	private static String mapsgoogle_apis;
	...
	@Override public Date newDate(){
		if (maintenant != 0) {
			shift++;
			return new Date(maintenantL + (shift * 1000L));
		}
		return new Date();
	}
	...
	@Override public String mail_password() {
		return mail_password;
	}

	@Override public String mapsgoogle_apis() {
		return mapsgoogle_apis;
	}
	...
	private void loadMailProps(){
		...
		try {
			...
			props2.put("mail.smtp.host", props.getProperty("mail.smtp.host"));
			props2.put("mail.smtp.auth", props.getProperty("mail.smtp.auth"));
			props2.put("mail.smtp.starttls.enable", props.getProperty("mail.smtp.starttls.enable"));
			props2.put("mail.smtp.port", props.getProperty("mail.smtp.port"));
			session = Session.getInstance(props2, new Authenticator() {
				@Override
				protected PasswordAuthentication getPasswordAuthentication() {
					return new PasswordAuthentication(mailUsername, mail_password());
				}
			});
		} catch (IOException e) {
	...
	@Override public void reloadConfig(AcJSONObject arg, Properties keys){
		try {
		...
		if (base.startsWith("http")) {
			try {
				...
				if (!ping) {
					...
					StringBuffer sb = new StringBuffer();
					sb.append("mailer=");
					sb.append(URLEncoder.encode(mailer, StandardCharsets.UTF_8));
					sb.append("&to=");
				...
				while (true) {
					...
					if (status == 200) {
						...
						String ret = new String(res, StandardCharsets.UTF_8);
						...
						try {
							...
							if (response != null) {
								...
								ret += new String(res, StandardCharsets.UTF_8);
							} else
								ret += "illisible";
						} catch(Exception e) {
			...
			return mailTo(mailer, to, subject, text);
		} else {
	...
	private static String mailTo(String mailer, String to, String subject, String text) {
		if (to == null || to.length() == 0 || subject == null || subject.length() == 0)
			return "KO: échec de l'envoi du mail";
		try {
