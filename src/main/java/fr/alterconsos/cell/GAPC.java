		...
		if (initG != null) {
			...
			for(GContact c : lst){
				...
				if (em != null && em.contains(filter)) {
					...
					String x = "[" + initG + "]\t[" + c.initiales + "]\t" + c.nom + "\t" + sb2 + "\n";
					for(String s : emx) {
...
//					sb.append("[" + initG + "] [" + c.initiales + "] " + c.nom + " ==> " + em2);
	...
	private static final String[] confids = {"normale", "restreinte", "publique"};

	private static final String[][] sheetNames = {{"Alterconsos"}, {"Producteurs"}};
