...
public class TaskMail extends Task {
	...
	private final ArrayList<IW> outArgs = new ArrayList<IW>();
	...
	private void startTask(AppTransaction tr, TodoList l, int retry){
		try {
			...
			if (retry != 0){
				...
				try { Thread.sleep(n * 60000L);} catch (InterruptedException e) {}
			}
	...
	private String doSend(AppTransaction tr) throws MailServerException, AppException {
		...
		for(Ctc c : contacts) {
			...
			if (maildelai != 0)
				try { Thread.sleep(maildelai * 1000L);} catch (InterruptedException ex) {}
